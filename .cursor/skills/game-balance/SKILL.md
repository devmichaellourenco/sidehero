...
- **Inimigos:** aba para editar identidade e skills de monstro por tipo; filtra por nome/id; roster completo com tier/role. Grava em `src/domain/enemies/data/enemy-combat-overrides.json`; merge em `EnemyCombatOverrides` → `EnemyCombatIdentityCatalog` / `CombatSkillRegistry`. API `GET|PUT /api/enemy-combat` + restore de backups.
